...
| What the household sees | Wait screen, then Seerr on Ingress | [`tests/preview.py`](tests/preview.py) is the wait screen. [`tests/preview_seerr_ingress.py`](tests/preview_seerr_ingress.py) is real Seerr behind an Ingress-path stripper. Seerr’s image is Alpine/musl; CI unpacks it and runs the image’s own `node` via `chroot` (host glibc node cannot load Seerr’s sqlite3 addon). |
...
python3 tests/preview_seerr_ingress.py   # real Seerr /setup at http://127.0.0.1:18099/api/hassio_ingress/tok/setup
```
