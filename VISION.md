...
`pompey/` is the Home Assistant app for the whole stack. The household guide (install, first run, which URLs you open, updates, what is not ready, roadmap) is the [root README](README.md). `0.2.38` is the current cut on real Home Assistant OS. Search offers Max / Default / Anything quality on the request (applied on existing installs, not only first boot). Recyclarr applies TRaSH HD to Default and TRaSH UHD (4K encodes, 1080p fallback, remux off) to Max. Language is Arr original-audio scoring, not Home Assistant options. Finished downloads on the same NAS share are renamed into the library folders; torrents whose files were already moved are dropped from the hidden download engine so they cannot start again. Engine auto-update, and Bazarr for missing subtitles, are still later. How those engines update without Pompey, why we will fetch artifacts instead of turning on Arr’s in-app updater, and what will go brittle when binaries start moving: [docs/arr-auto-update.md](docs/arr-auto-update.md).
