...
`pompey/` is the Home Assistant app for the whole stack. The household guide (install, first run, which URLs you open, updates, what is not ready, roadmap) is the [root README](README.md). `0.2.51` is the current cut on real Home Assistant OS. The sidebar is a dashboard once search is wired (not a first-boot bar that keeps filling because you opened Ingress). Search offers Max / Default / Anything quality on the request (applied on existing installs, not only first boot). Recyclarr applies TRaSH HD to Default and TRaSH UHD (4K encodes, 1080p fallback, remux off) to Max. Language is Arr original-audio scoring plus a Dual-Audio tie-break, not Home Assistant options. Finished downloads on the same NAS share are renamed into the library folders; torrents whose files were already moved are dropped from the hidden download engine so they cannot start again. Engines (including Recyclarr’s binary) refresh from official channels on start and about daily; Pompey’s add-on version is a separate clock. Bazarr for missing subtitles is still later. How those engines update without Pompey, why we fetch artifacts instead of turning on Arr’s in-app updater, and what goes brittle when binaries move: [docs/arr-auto-update.md](docs/arr-auto-update.md). Whether we could drop Radarr/Sonarr and fake their API for Seerr: [docs/replace-arr.md](docs/replace-arr.md) (design note — keep Arr; steal search before replacing the matcher).
