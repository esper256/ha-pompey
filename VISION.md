...
`pompey/` is the Home Assistant app for the whole stack. The household guide (install, first run, which URLs you open, updates, what is not ready, roadmap) is the [root README](README.md). `0.2.29` is the current cut on real Home Assistant OS. Search offers Max / Default / Anything quality on the request. Finished downloads import into the library folders; torrents whose files were already moved are dropped from the hidden download engine so they cannot start again. Live Recyclarr sync of TRaSH scores, and Bazarr for missing subtitles, are still later.
