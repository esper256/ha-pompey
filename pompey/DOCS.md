...
Home Assistant options are the media folder, four library folders relative to it, and **after a title is in the library** (stop sharing by default). Language is not an HA option — Recyclarr/TRaSH set original audio on Default and Max, with Dual-Audio as a small tie-break. Plex is Seerr’s first screen. Sources are Prowlarr on **9696**. Proton is paste on the wait screen. WireGuard internals are fixed: Proton DNS `10.2.0.1`, RFC1918 LAN plus Supervisor `172.30.32.0/23`.
