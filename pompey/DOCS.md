...
The kill switch is **OUTPUT** (internet from this app). Home Assistant Ingress to port 8099 and LAN clients to published **5055** are **INPUT** from Supervisor / Docker and are not blocked. If the tunnel cannot start, the wait screen stays up; the container is not halted.
...
Ingress **8099** is always Pompey's wait/status UI. Search is Seerr on published host port **5055** (no path rewriter). Putting Seerr under Ingress breaks on the next minified Next.js chunk.
...
Do not publish download peer ports on the Home Assistant host. Leave **5055** published for Seerr.
