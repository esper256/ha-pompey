...
**0.2.20** is the current cut on a real Home Assistant OS machine. Sidebar is Pompey (Proton, status, Open search, Open sources). Search is Seerr on port 5055. Sources are Prowlarr on port 9696. Plex is a separate app. Media folders are Home Assistant options. A title landing in Plex is still unproven. See the README for the intended journey, what is not ready, and the roadmap.
