...
| Category `savePath` = `downloads/complete` | `qbit_category` (`radarr` / `sonarr` / `prowlarr`) | AutoTMM or a new default save path drops files into `incomplete/` forever, or into a library folder. Prowlarr Grab must not reuse `radarr`/`sonarr` or Arr CDH mis-imports. |
