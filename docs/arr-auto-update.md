...
| Category `savePath` | `qbit_category`: `radarr`/`sonarr` → `downloads/complete`; `prowlarr` → `downloads/manual` | AutoTMM or a new default save path drops files into `incomplete/` forever, or into a library folder. Prowlarr Grab must not reuse `radarr`/`sonarr` or Arr CDH mis-imports. |
...
| `GET /api/v1/request` + `wanted/cutoff` + unmonitor PUT | `improve_existing_requests` | Seerr request list 403/shape change: we skip (no unmonitor, no cutoff poke). Arr cutoff path rename: no reconsider after a new source. |
...
| Download-client schema field names `host` / `username` / `movieCategory` | `qbit_client_values` | Client never points at localhost qbit. Prowlarr uses `/api/v1/downloadclient` and category `prowlarr`. |
