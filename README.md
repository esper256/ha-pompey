...
- Download finished but the file is still under `downloads/complete` (including a leftover `complete/radarr` folder): rebuild to **0.2.32**. **0.2.27** stopped skipping the library move when a network share reports no free space. **0.2.29** forgets the hidden qBittorrent torrent if the file was already moved. Do not drag files out of `complete/` yourself. Plex only sees files that made it into the library folders.
