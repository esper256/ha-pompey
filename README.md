...
Release names that include an executable, a script, an archive (`.zip`, `.rar`, `.7z`), or a disc image (`.iso`) are ignored in Radarr, Sonarr, and Prowlarr. qBittorrent skips those file types on new downloads. A transfer with no playable video is removed, including while it is still in the incomplete folder, and blocklisted. Split archive parts, release notes, and sample clips do not keep it. A real video that arrives beside one of those files keeps the video and its subtitles; the extra file is removed.
