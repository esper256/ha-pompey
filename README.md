...
Version **0.2.4**. We do not publish a container image. Supervisor builds this app on the machine. After the tunnel is up, the app fetches the official programs it needs.
...
| Proton + kill switch | WireGuard from that pasted Proton file; `PersistentKeepalive`; internet OUTPUT only on `wg0` (iptables-legacy); RFC1918 LAN (Plex, NAS) allowed |
