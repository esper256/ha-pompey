...
Version **0.2.12**. We do not publish a container image. Supervisor builds this app on the machine. After the tunnel is up, the app fetches the official programs it needs.
...
| Local wiring | Engines on localhost; Prowlarr syncs the source to TV/movie engines; Seerr pointed at them and auto-approves. If Plex is missing, a local Seerr account is created so TV/movie engines still get wired. A required miss (indexer, Seerr→Radarr/Sonarr) keeps the wait screen up |
