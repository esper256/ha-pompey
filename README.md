...
This is the user guide for the product we are building. It describes the journey as it should feel. [What is not ready yet](#what-is-not-ready-yet) is honest about the current cut (**0.2.52**). The [roadmap](#roadmap) is how we close the gap.
...
- **A title that will not grab itself.** Request it in search first so Radarr or Sonarr already has the title and the Kid / Not Kid / By Rating folder. Then turn on **Debug**, restart, open **Radarr** or **Sonarr** from the sidebar, and use **Interactive Search** on that title. The grab uses Arr’s own qBittorrent client (`radarr` / `sonarr` category) and lands in the folder Seerr stored. **Open sources → Search → Grab** is the fallback: Prowlarr talks to the same qBittorrent, but with a separate **`prowlarr`** category so a TV grab cannot be imported as a movie. Housekeeping only moves the file if Arr already has that title — it does not guess Kid vs Not Kid from the filename. Do not Grab a title you never requested; it sits in `downloads/complete`. Do not edit those download-client settings by hand — a restart rewrites them.
...
| Language, subtitles, dual audio | **0.2.41** uses Arr’s original-audio custom format (not Home Assistant options, not a Seerr language picker). Default/Max skip a dub when the original exists, and prefer Dual-Audio as a small tie-break when both exist. **0.2.43** will search again for a better copy after you add a source (and when a later Dual-Audio release shows up), until you remove the Seerr request. **0.2.51** season-searches a holey anime season (packs) instead of only `Title 28` episode queries, and turns on Prowlarr’s Anime Standard Format Search so those pack queries actually go out. Anything still takes whatever is left. Missing subtitles after the file lands are later (Bazarr). Playback language is Plex. |
