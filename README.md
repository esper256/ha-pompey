...
This is the user guide for the product we are building. It describes the journey as it should feel. [What is not ready yet](#what-is-not-ready-yet) is honest about the current cut (**0.2.35**). The [roadmap](#roadmap) is how we close the gap.
...
- Quality profiles: **0.2.28** puts three choices on the Seerr request: Max, Default, Anything. **0.2.31** is the cut that can actually apply them on this install (earlier cuts died on a Sonarr free-space field before quality ran). Recyclarr is not yet refreshing TRaSH scores on a schedule.
...
The journey above is the target. **0.2.35** is a real Home Assistant OS install of that box, not the finished household app.
...
3. **Quality profiles (Recyclarr / TRaSH).** **0.2.28** is Max / Default / Anything on the Seerr request, plus Home Assistant language, anime-audio, and subtitle scoring of release names. Next is keeping TRaSH scores current (Recyclarr or equivalent) when engines update, still off the sidebar. **Bazarr** (download missing English subtitles after the file is in the library) is a later engine in the same container — not a Seerr fork, and not a per-request language dropdown (Seerr does not have one; Sonarr language profiles are gone).
...
- A TV season searches some episodes in **Open sources** then sits: rebuild to **0.2.35**. The app log will show Sonarr’s command queue and wanted/missing *before* housekeeping asks it to scan `complete/` again, without Seerr debug Plex-scan or Arr stack-trace dumps. Send that stretch of the log (not API keys).
- Download finished but leftover junk is still under `downloads/complete` after the title is in Plex: rebuild to **0.2.34**. Arr imports the video (and `.srt` / `Subs`) into the movie or TV folder Seerr stored — not the whole torrent dump. Housekeeping then removes that leftover folder. Do not drag files out of `complete/` yourself. Do not use Seerr’s red Remove / Clear Data buttons for this. Plex only sees files that made it into the library folders.
