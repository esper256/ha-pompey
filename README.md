...
- Download finished but the file is still under `downloads/complete` (including a leftover `complete/radarr` folder, or a new grab that never left `complete/`): rebuild to **0.2.32**. That cut stops the hidden torrent so the NAS file is not locked, then **renames** it into the Plex library folder on the same share. **0.2.27** stopped skipping the library move when a network share reports no free space. **0.2.29** forgets the hidden qBittorrent torrent if the file was already moved. Do not drag files out of `complete/` yourself. Do not use Seerr’s red Remove / Clear Data buttons for this. Plex only sees files that made it into the library folders.
