...
This is the user guide for the product we are building. It describes the journey as it should feel. [What is not ready yet](#what-is-not-ready-yet) is honest about the current cut (**0.2.31**). The [roadmap](#roadmap) is how we close the gap.
...
The journey above is the target. **0.2.31** is a real Home Assistant OS install of that box, not the finished household app.
...
If search is a blank page on port 5055, or the Plex button on setup does nothing, that is a bug — send the log. Rebuild so the banner says **0.2.31** if the wait screen says wiring failed (including Sonarr minimum free space ≥ 100) and the request quality list is still Any / HD-720p / Ultra-HD. Rebuild to **0.2.29** if a finished download stayed in `downloads/complete`, or you moved a file by hand and worry qBittorrent will grab it again. Also rebuild if auto-grab picked a huge remux on Default, a Seerr request only title-searched two Prowlarr sources, or you still need tagged app logs, household media-folder defaults, **Open sources**, or an older wait screen.
...
- Download finished but the file is still under `downloads/complete`: rebuild to **0.2.29**. **0.2.27** stopped skipping the library move when a network share reports no free space. **0.2.29** also forgets the hidden qBittorrent torrent if the file was already moved (it does not delete library files, and it does not start the download again). Do not drag files out of `complete/` yourself. Plex only sees files that made it into the library folders.
