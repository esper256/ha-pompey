...
This is the user guide for the product we are building. It describes the journey as it should feel. [What is not ready yet](#what-is-not-ready-yet) is honest about the current cut (**0.2.38**). The [roadmap](#roadmap) is how we close the gap.
...
The journey above is the target. **0.2.38** is a real Home Assistant OS install of that box, not the finished household app.
...
| Engines stay current for years | First fetch only. Already-present binaries (including Recyclarr) are skipped on restart. Arr’s own updater is off (`UpdateAutomatically=False`, mechanism Docker). Months of uptime freeze Radarr at the first download. Recyclarr still re-syncs TRaSH JSON through the tunnel. The plan is **not** to turn Arr’s in-app updater on, and **not** to bump Pompey for every Radarr release — Pompey should fetch new official artifacts on a timer the way Docker compose pulls images. Decision and native-update table: [docs/arr-auto-update.md](docs/arr-auto-update.md). |
...
2. **Engine and Seerr updates.** Pompey plays the Docker-orchestrator role: fetch official artifacts (Servarr update APIs, Seerr’s published image, qBittorrent-nox static builds, Recyclarr GitHub releases), replace on-disk copies when upstream moves, restart those processes, re-run wire. Check on add-on start *and* on a timer so a wrapper we do not touch for months still refreshes Radarr. Never require the user to open an engine UI to click Update. Never bump the Pompey add-on version just because Radarr tagged a release. Do **not** enable Arr’s BuiltIn updater inside this container (Servarr’s own Docker advice). Design, native-update table, rejected options: [docs/arr-auto-update.md](docs/arr-auto-update.md). **Today this is not done:** first fetch wins, Arr self-update is disabled.
3. **Quality profiles (Recyclarr / TRaSH).** **0.2.38** is Max / Default / Anything on the Seerr request, with Recyclarr applying TRaSH HD to Default and TRaSH UHD (plus 1080p fallback, remux off) to Max. Anything stays a Pompey profile. Next is keeping Recyclarr itself current when engines update, still off the sidebar. **Bazarr** (download missing English subtitles after the file is in the library) is a later engine in the same container — not a Seerr fork, and not a per-request language dropdown (Seerr does not have one; Sonarr language profiles are gone).
