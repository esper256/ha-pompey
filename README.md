...
Copy `pompey/` into `/addons` (this GitHub repo is private, so Home Assistant cannot clone it as an Apps repository without a login). Let Supervisor build, fill Proton + Plex IP + token + source, start **Pompey**, open the sidebar. First start can take several minutes. If the wait screen stays on the tunnel step, Proton is not up. Operator checklist (Plex token, Plex libraries): [pompey/DOCS.md](pompey/DOCS.md).
