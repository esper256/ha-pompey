...
This is the user guide for the product we are building. It describes the journey as it should feel. [What is not ready yet](#what-is-not-ready-yet) is honest about the current cut (**0.2.50**). The [roadmap](#roadmap) is how we close the gap.
...
The journey above is the target. **0.2.50** is a real Home Assistant OS install of that box, not the finished household app.
...
| Language, subtitles, dual audio | **0.2.41** uses Arr’s original-audio custom format (not Home Assistant options, not a Seerr language picker). Default/Max skip a dub when the original exists, and prefer Dual-Audio as a small tie-break when both exist. **0.2.43** will search again for a better copy after you add a source (and when a later Dual-Audio release shows up), until you remove the Seerr request. Anything still takes whatever is left. Missing subtitles after the file lands are later (Bazarr). Playback language is Plex. |
