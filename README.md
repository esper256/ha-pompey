...
This is the user guide for the product we are building. It describes the journey as it should feel. [What is not ready yet](#what-is-not-ready-yet) is honest about the current cut (**0.2.22**). The [roadmap](#roadmap) is how we close the gap.
...
The journey above is the target. **0.2.22** is a real Home Assistant OS install of that box, not the finished household app.
...
If search is a blank page on port 5055, or the Plex button on setup does nothing, that is a bug — send the log. Rebuild so the banner says **0.2.22** if the wait screen is stuck after a media-folder change, or if you still need the household media-folder defaults, **Open sources**, or an older wait screen.
